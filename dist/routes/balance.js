...
router.post('/update', async (req, res) => {
    try {
        ...
        const balanceOwner = await User_1.default.getBalanceOwner(userIdToProcess);
        if (!balanceOwner)
            return res.status(404).json({ message: 'Balance owner not found' });
        if (method === 'Cash') {
