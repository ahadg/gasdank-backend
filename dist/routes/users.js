...
router.get('/', authMiddleware_1.authenticateJWT, (0, accessMiddleware_1.default)("config.users", "read"), async (req, res) => {
    ...
    try {
        ...
        if (the_user.role === "superadmin") {
            ...
            users = await User_1.default.find({ created_by: req.user?.id });
        }
...
router.get('/:id', authMiddleware_1.authenticateJWT, (0, accessMiddleware_1.default)("config.users", "read"), async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/', authMiddleware_1.authenticateJWT, (0, accessMiddleware_1.default)("config.users", "create"), async (req, res) => {
    try {
        ...
        const hashedPassword = await bcrypt_1.default.hash(value.password, saltRounds);
        value.password = hashedPassword;
        value.created_by = req.user?.id;
        // Create and save the new user
        const newUser = new User_1.default(value);
        await newUser.save();
        // create activity 
...
router.post('/signup', async (req, res) => {
    try {
        ...
        const { error, value } = user_1.userSignupSchema.validate(req.body);
        if (error) {
            return res.status(400).json({ error: error.details[0].message });
        }
        ...
        value.access = helpers_1.adminDefaultAccess;
        value.role = 'admin';
        value.created_at = new Date();
        value.updated_at = new Date();
        // Validate selected plan
        const systemSettings = await SystemSettings_1.default.findOne();
        const validPlan = systemSettings?.plans?.find((p) => p.name.toLowerCase() === value.plan.toLowerCase());
        if (!validPlan) {
...
router.patch('/:id', authMiddleware_1.authenticateJWT, (0, accessMiddleware_1.default)("config.users", "edit"), async (req, res) => {
    try {
        const { id } = req.params;
...
router.put('/', authMiddleware_1.authenticateJWT, async (req, res) => {
    try {
...
router.put('/:user_id', authMiddleware_1.authenticateJWT, async (req, res) => {
    try {
        const { user_id } = req.params;
        const updatedUser = await User_1.default.findByIdAndUpdate(user_id, req.body, { new: true });
        res.status(200).json(updatedUser);
    }
    catch (error) {
        res.status(500).json({ error });
...
router.delete('/', authMiddleware_1.authenticateJWT, (0, accessMiddleware_1.default)("config", "delete"), async (req, res) => {
    try {
        const { id } = req.body;
        await User_1.default.findByIdAndUpdate(id, { deleted_at: new Date() });
...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        const { user, rawToken } = await (0, passwordReset_1.setUserResetToken)(email);
        if (!user) {
        ...
        await (0, sendEmail_1.sendPasswordEmail)(email, resetLink);
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        const user = await (0, passwordReset_1.verifyTokenAndUpdatePassword)(token, password);
        if (!user)
...
router.get("/stats/:user_id", authMiddleware_1.authenticateJWT, (0, accessMiddleware_1.default)("dashboard", "read"), async (req, res) => {
    try {
        const { user_id } = req.params;
        if (!user_id) {
        ...
        const the_user = await User_1.default.findById(userObjectId);
        if (!the_user) {
        ...
        const groupUsers = await User_1.default.find({ $or: [{ _id: rootAdminId }, { created_by: rootAdminId }] }, { _id: 1, cash_balance: 1, other_balance: 1 }).lean();
        const userIds = groupUsers.map((u) => u._id);
        // Aggregate balances across the group
        let groupCashBalance = 0;
        let groupOtherBalance = {};
        ...
        // === AGGREGATIONS ===
        // Total Sales
        const totalSalesAgg = await Transaction_1.default.aggregate([
            {
                $match: {
                    user_id: { $in: userIds },
                    type: "sale",
        ...
        // Total Return
        const totalSalesProductReturnAgg = await Transaction_1.default.aggregate([
            {
                $match: {
                    user_id: { $in: userIds },
                    type: "return",
        ...
        // Total Profit
        const totalProfitAgg = await Transaction_1.default.aggregate([
            {
                $match: {
                    user_id: { $in: userIds },
                    type: "sale",
        ...
        // Profit Return
        const totalProductReturnProfitAgg = await Transaction_1.default.aggregate([
            {
                $match: {
                    user_id: { $in: userIds },
                    type: "return",
        ...
        const totalProfitReturn = totalProductReturnProfitAgg[0]?.totalProfit || 0;
        // Inventory Value
        const inventoryValueAgg = await Inventory_1.default.aggregate([
        ...
        const clientPayableAgg = await Buyer_1.default.aggregate([
            {
                $match: {
        ...
        const companyPayableAgg = await Buyer_1.default.aggregate([
            {
                $match: {
                    $or: [{ user_id: { $in: userIds } }, { admin_id: { $in: userIds } }],
                    currentBalance: { $lt: 0 },
        ...
        const rawCompanyBalance = rawInventoryValue +
...
router.delete("/clean-data/:user_id", authMiddleware_1.authenticateJWT, 
...
async (req, res) => {
    try {
        ...
        const userObjectId = new mongoose_1.default.Types.ObjectId(user_id);
        // Verify user exists
        const user = await User_1.default.findById(userObjectId);
        if (!user) {
            return res.status(404).json({ error: "User not found" });
        }
        ...
        let childUserIds = [];
        if (isAdminRequest) {
            const childUsers = await User_1.default.find({ created_by: userObjectId }, { _id: 1 });
            childUserIds = childUsers.map((u) => u._id);
        }
        ...
        try {
            // Delete Activities
            const activityResult = await Activity_1.default.deleteMany({
                $or: [
        ...
        try {
            // Delete Buyers
            const buyerResult = await Buyer_1.default.deleteMany({
                $or: [
                    { user_id: { $in: allTargetUserIds } },
        ...
        try {
            // Delete Expenses
            const expenseResult = await Expense_1.default.deleteMany({
                $or: [
                    { user_id: { $in: allTargetUserIds } },
        ...
        try {
            // Delete Inventory
            const inventoryResult = await Inventory_1.default.deleteMany({
                $or: [
                    { user_id: { $in: allTargetUserIds } },
                    { user_created_by_id: { $in: allTargetUserIds } }
                ]
        ...
        try {
            ...
            const notificationResult = await notification_1.default.deleteMany({
                $or: [
                    { user_id: { $in: allTargetUserIds } },
        ...
        try {
            // Delete Samples
            const sampleResult = await Sample_1.default.deleteMany({
                user_id: { $in: allTargetUserIds }
            });
        ...
        try {
            ...
            const sampleViewingResult = await SampleViewingClients_1.default.deleteMany({
                $or: [
                    { user_id: { $in: allTargetUserIds } },
                    { user_created_by: { $in: allTargetUserIds } }
                ]
        ...
        try {
            ...
            const transactionItemResult = await TransactionItem_1.default.deleteMany({
                $or: [
                    { user_id: { $in: allTargetUserIds } },
                    { admin_id: { $in: allTargetUserIds } }
                ]
        ...
        try {
            ...
            const transactionPaymentResult = await TransactionPayment_1.default.deleteMany({
                $or: [
                    { user_id: { $in: allTargetUserIds } },
                    { admin_id: { $in: allTargetUserIds } }
                ]
        ...
        try {
            ...
            const userUpdateResult = await User_1.default.findByIdAndUpdate(userObjectId, {
                $set: {
                    cash_balance: 0,
                    other_balance: {}
                }
            ...
            if (isAdminRequest && childUserIds.length > 0) {
                const userDeleteResult = await User_1.default.deleteMany({ _id: { $in: childUserIds } });
