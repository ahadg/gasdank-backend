...
exports.formatPhoneNumber = exports.createNotification = exports.twilioClient = void 0;
...
const createNotification = async (obj) => {
    const newNotification = new notification_1.default(obj);
...
function generateMessage(type, customMessage, data) {
    ...
    if (type === 'outstanding') {
        ...
        const productList = data.products.map((p) => `${p.name} ($${p.price})`).join(', ');
...
function generateEmailHtml(type, message, data) {
    if (type === 'outstanding') {
        return `
        ...
        for (const recipient of recipients) {
            try {
                ...
                if (type === 'product' && products) {
                    notificationData.products = products.map((p) => ({
                        product_id: p._id,
                        product_name: p.name,
                ...
                if (communicationMethod === 'sms') {
                    ...
                    if (phone) {
                        const formattedPhone = (0, exports.formatPhoneNumber)(phone);
                        if (formattedPhone) {
                            try {
                                communicationResult = await exports.twilioClient.messages.create({
                                    body: messageContent,
                ...
                const savedNotification = await (0, exports.createNotification)(notificationData);
...
router.post('/send-invoice/:buyer_id', async (req, res) => {
    try {
        const { buyer_id } = req.params;
        const { customMessage, totalAmount, dueAmount, transactionCount, dateRange } = req.body;
        const user_id = req.user?.id || req.user?._id;
        // Validation
        if (!buyer_id) {
            return res.status(400).json({
        ...
        const client = await Buyer_1.default.findById(buyer_id); // Replace 'User' with your actual client model
        if (!client) {
        ...
        const generateInvoiceMessage = (customMsg) => {
            if (customMsg) {
                return customMsg;
            }
        ...
        if (totalAmount !== undefined)
            notificationData.invoice_total = totalAmount;
        if (dueAmount !== undefined)
            notificationData.invoice_due = dueAmount;
        if (transactionCount !== undefined)
            notificationData.transaction_count = transactionCount;
        if (dateRange)
            notificationData.date_range = dateRange;
        try {
            // Send SMS
            const smsResult = await exports.twilioClient.messages.create({
                body: messageContent,
                from: process.env.TWILIO_PHONE_NUMBER,
                to: formattedPhone
            });
...
router.get('/:user_id', async (req, res) => {
    try {
        ...
        const totalNotifications = await notification_1.default.countDocuments(query);
...
router.put('/:notification_id/read', async (req, res) => {
    try {
        ...
        const notification = await notification_1.default.findByIdAndUpdate(notification_id, {
            isRead: true,
            updated_at: new Date()
        }, { new: true });
        if (!notification) {
...
router.put('/:user_id/mark-all-read', async (req, res) => {
    try {
        const { user_id } = req.params;
        const result = await notification_1.default.updateMany({ user_id, isRead: false }, {
            isRead: true,
            updated_at: new Date()
        });
        res.status(200).json({
...
router.delete('/:notification_id', async (req, res) => {
    try {
        const { notification_id } = req.params;
        const notification = await notification_1.default.findByIdAndDelete(notification_id);
        if (!notification) {
            return res.status(404).json({ error: 'Notification not found' });
        }
...
router.delete('/:user_id/delete-all', async (req, res) => {
    try {
        const { user_id } = req.params;
        const result = await notification_1.default.deleteMany({ user_id });
        res.status(200).json({
