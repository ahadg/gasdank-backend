...
exports.getSuggestionColor = getSuggestionColor;
...
router.get('/sparkline-data', async (req, res) => {
    try {
        ...
        const salesProfitData = await Transaction_1.default.aggregate([
            {
                $match: {
        ...
        const expenseData = await Expense_1.default.aggregate([
            {
                $match: {
                    user_id: new mongoose_1.default.Types.ObjectId(userId),
                    created_at: { $gte: last30Days }
        ...
        const salesProfitMap = new Map(salesProfitData.map(item => [item._id, { sales: item.sales, profit: item.profit }]));
        const expenseMap = new Map(expenseData.map(item => [item._id, item.expenses]));
...
router.get('/monthly-revenue', async (req, res) => {
    try {
        ...
        if (startDate || endDate) {
            ...
            if (startDate)
                matchConditions.created_at.$gte = new Date(startDate);
            if (endDate)
                matchConditions.created_at.$lte = new Date(endDate);
        }
        ...
        if (product && product !== '') {
            pipeline.unshift({
                ...
                $match: {
                    'items.inventory_id': new mongoose_1.default.Types.ObjectId(product)
                }
            });
        }
        const monthlyData = await Transaction_1.default.aggregate(pipeline);
...
router.get('/top-categories', async (req, res) => {
    try {
        ...
        const topCategories = await TransactionItem_1.default.aggregate([
            { $match: matchConditions },
            {
                $lookup: {
                    from: 'inventories',
...
router.get('/top-products', async (req, res) => {
    try {
        ...
        const topProducts = await TransactionItem_1.default.aggregate([
            { $match: matchConditions },
            {
                $lookup: {
                    from: 'inventories',
...
router.get('/top-clients', async (req, res) => {
    try {
        ...
        }, { $sort: { totalSales: -1 } }, { $limit: parseInt(limit) });
        const topClients = await Transaction_1.default.aggregate(pipeline);
        // Format data
...
router.get('/payment-types', async (req, res) => {
    try {
        ...
        const paymentTypesData = await Transaction_1.default.aggregate([
            {
                $match: matchConditions
            },
            {
...
router.get('/forecasting', async (req, res) => {
    try {
        ...
        const recentSales = await Transaction_1.default.aggregate([
            {
                $match: {
                    user_id: new mongoose_1.default.Types.ObjectId(userId),
                    deleted_at: null,
        ...
        const productSales = await Transaction_1.default.aggregate([
            {
                $match: {
                    user_id: new mongoose_1.default.Types.ObjectId(userId),
                    deleted_at: null,
        ...
        const detectSeasonality = (data) => {
            if (data.length < 3)
                return { seasonal: false, factor: 1 };
        ...
        const calculateRealisticForecast = (historicalData, metric) => {
            ...
            for (let i = 1; i <= forecastMonths; i++) {
                ...
                let combinedPrediction = (regPrediction * regressionWeight +
            ...
            let confidence = Math.round((regression.rSquared * 50 + dataQuality * 25 + 35));
        ...
        const monthlyArray = Object.values(monthlyData).sort((a, b) => a.year - b.year || a.month - b.month);
        ...
        for (let i = 1; i <= forecastMonths; i++) {
            ...
            const avgConfidence = Math.round((revenueForecast.confidence + profitForecast.confidence + transactionForecast.confidence) / 3);
            forecasts.push({
...
router.get('/low-stock-products', async (req, res) => {
    try {
        ...
        const stockAnalysis = await TransactionItem_1.default.aggregate([
            {
                $match: {
                    user_id: new mongoose_1.default.Types.ObjectId(userId),
                    deleted_at: null,
...
router.post('/ai-suggestions', async (req, res) => {
    try {
        ...
        const [topProducts, topCategories, recentTrends, lowStock, totalRevenue, transactionCount] = await Promise.all([
            // Top products
            TransactionItem_1.default.aggregate([
                {
                    $match: {
                        user_id: new mongoose_1.default.Types.ObjectId(userId),
                        deleted_at: null,
...
${context.topProducts.slice(0, 5).map((p) => `- ${p.name}: $${p.revenue.toFixed(2)} revenue, ${p.quantity} units sold, avg $${p.avgPrice.toFixed(2)} per unit`).join('\n')}

Top Categories:
${context.topCategories.map((c) => `- ${c.name}: $${c.revenue.toFixed(2)} revenue, ${c.quantity} units sold`).join('\n')}

Monthly Revenue Trends:
${context.monthlyTrends.map((t) => `- ${t.month}: $${t.revenue.toFixed(2)} revenue, ${t.transactions} transactions`).join('\n')}

Low Stock Items:
${context.lowStockItems.map((s) => `- ${s.name}: ${s.currentStock} in stock, ${s.soldQuantity} sold recently ($${s.revenue.toFixed(2)} revenue)`).join('\n')}
...
Focus on actionable, data-driven insights that can directly improve revenue, reduce costs, or optimize operations. Consider inventory management, product performance, pricing optimization, marketing opportunities, and operational efficiency.`;
    try {
        ...
        const validatedSuggestions = parsedSuggestions
            .filter((s) => s.title && s.description && s.action && s.impact)
            .map((s) => ({
            type: s.type || 'general',
...
function generateBusinessSuggestions(context) {
    const suggestions = [];
    ...
    if (context.lowStockItems.length > 0) {
        const totalLowStockRevenue = context.lowStockItems.reduce((sum, item) => sum + item.revenue, 0);
        suggestions.push({
            type: 'inventory',
            priority: 'high',
            ...
            action: `Immediately reorder: ${context.lowStockItems.slice(0, 3).map((item) => item.name).join(', ')}`,
    ...
    if (context.topProducts.length > 0) {
        ...
        if (revenueShare > 30) {
            ...
            suggestions.push({
                ...
                action: `Increase marketing spend on top 3 products: ${context.topProducts.slice(0, 3).map((p) => p.name).join(', ')}`,
                impact: `Potential revenue increase of $${(context.topProducts.slice(0, 3).reduce((sum, p) => sum + p.revenue, 0) * 0.2).toFixed(2)} monthly`
            });
        }
    }
